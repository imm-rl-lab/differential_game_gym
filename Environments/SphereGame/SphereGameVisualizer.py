...
from Environments.SphereGame import SphereGameSolution as solution


class SphereGameVisualizer:
    ...
    def show_fig(self, env, u_agent, v_agent, sessions):
        ...
        true_value = solution.value_function(env, env.reset())
        
        plt.figure(figsize=[18, 12])

        plt.subplot(231)
        phi = np.linspace(0, 2 * np.pi, 50)
        plt.plot(4 * np.sin(phi), 4 * np.cos(phi), 'r', label='Terminal set')
        ...
        plt.plot([true_value] * len(self.total_rewards), 'r', label='True value')
        plt.legend()
        plt.grid()
